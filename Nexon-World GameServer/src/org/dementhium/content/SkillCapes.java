	...
	public static boolean handleDialogue(Player player, int stage) {
		...
		case 579:
			if(player.getSkills().getLevel(master.skillId) < 98){
				...
				DialogueManager.sendDialogue(player, DialogueManager.HAPPY_TALKING, master.getNpcId(), 580, "Most certainly But it's gonna cost ya.","I'm thinkin somewhere around 99K aight nigga?","That sound all good to you?");
			}
			return true;
		case 580:
			DialogueManager.sendOptionDialogue(player, new int[] {581, 582}, "99K? I Would but I'm fucking poor", "Uhh hell yea I want that fucking cape!");
			return true;
		case 581:
			DialogueManager.sendDialogue(player, DialogueManager.DEPRESSED, -1, -1, "99K? That's too much for my poor ass :[ ");
			return true;
		case 582:
			DialogueManager.sendDialogue(player, DialogueManager.HAPPY_TALKING, -1, 583, "Uhh hell yeah I want that fucking","cape!");
			return true;
		case 583:
			if (player.getInventory().contains(995, 99000)) {
				...
				DialogueManager.sendDialogue(player, DialogueManager.HAPPY_TALKING, master.getNpcId(), -1, "Damnnnn you all maxed out homie!");
			} else {
				DialogueManager.sendDialogue(player, DialogueManager.SAD, master.getNpcId(), -1, "Haha you poor bitch these are 99k Each!");
			}
			return true;
		}
