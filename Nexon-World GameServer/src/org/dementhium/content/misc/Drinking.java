...
public class Drinking extends EventListener {
	...
	SLAYER_RESPITE(new int[]{5759, 1919}, new int[]{Skills.SLAYER}, PotionType.BEER_BREW),
    ...
    public boolean interfaceOption(final Player player, int interfaceId, int buttonId, int slot, int itemId, int opcode) {
		...
		case BEER_BREW:
			ActionSender.sendChatMessage(player, 0, "You drink the " + potionName + ".");
			for (int i = 0; i < drink.getSkills().length; i++) {
				int skill = drink.getSkill(i);
				int modification = (int) Math.floor(1 + (player.getSkills().getLevelForExperience(skill) * 0.1));
