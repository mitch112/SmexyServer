...
public class ObjectPacketHandler extends PacketHandler {
	...
	public static final Animation CLIMB_ANIMATION2 =  Animation.create(827);
	@Override
	public void handlePacket(final Player player, final Message packet) {
		...
		case 4039:
			...
			player.teleport(2209, 5348, 0);
				break;
		case 37631:
			if (player.getSkills().getLevel(14) >= 80) {
				player.teleport(2792, 4427, 0);
			} else {
				player.sendMessage("You need a mining level of atleast 80 to enter the mines.");
			}
			break;
		case 8749:
			DialogueManager.proceedDialogue(player, 42000);
			break;
		case 37990:
		...
		if  (!findObjectPath(player, gameObject).isRouteFound() && objectId != 4496 && objectId != 11666 && objectId != 9356 && objectId != 26288 && objectId != 26287 && objectId != 26286 && objectId != 18342 && objectId != 50604 && objectId != 50343 && objectId != 48496 && objectId != 4388 && objectId != 26445 && objectId != 26303 && objectId != 57225 && objectId != 25339 && objectId != 25340 && objectId != 25336 && objectId != 25338 && objectId != 2288 && objectId != 32048) {
	...
	private void handleObject(final Player player, int objectClicked, final GameObject gameObject, Location location, CacheObjectDefinition definition, Message packet) {
		...
		if (name.equals("bank booth") || name.equals("bank chest")) {
			...
			case 28714:
				...
				break;
			case 48208:
				if (gameObject.getLocation().getX() == 3328 && 
					gameObject.getLocation().getY() == 3161) {
					player.teleport(3327, 3161, 0);
				} else {
					player.teleport(3330, 3161, 0);
				}
				break;
		case 49345:
				if (player.getInventory().contains(8859)) {
			...
			case 27254:
				...
				if (gameObject.getLocation().getX() == 3782 && gameObject.getLocation().getY() == 3549) {
					player.teleport(3095, 3497, 0);
				} else {
					player.teleport(3784, 3549, 0);
				}
				break;
			case 9294:
				player.teleport(2878, 9813, 0);
				break;
			case 46300:
				if (gameObject.getLocation().getX() == 3323 && gameObject.getLocation().getY() == 3491) {
					ActionSender.sendInterface(player, 402);
				} else {
			...
			case 5960:
				...
				//REVS CAVE
				case 20600:
					player.teleport(3037, 10171, 0);
				break;
				case 18341:
					player.teleport(3071, 3649, 0);
		break;	
				case 42219:
				player.sendMessage("You step into portal, and have went to Soulwars Lobby.");
				player.teleport(1886, 3177, 0);
		break;	
		case 42220:
				player.sendMessage("You step into portal, and have went back to Edgeville.");
				player.teleport(3081, 3475, 0);
				break;
			case 5959:
				player.animate(2140);
			...
			case 2468:
				...
				break;
			case 2469:
				DialogueManager.proceedDialogue(player, 6950);
				break;
			case 2470:
