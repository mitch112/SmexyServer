...
public class NpcOption extends PacketHandler {
	...
	private void option3(final Player player, Message packet) {
		...
		World.getWorld().submitAreaEvent(player, new CoordinateEvent(player, locationToWalk.getX(), locationToWalk.getX(), npc.getDefinition().getCacheDefinition().size, npc.getDefinition().getCacheDefinition().size) {
			...
			public void execute() {
				int id = npc.getId();
				if (id > 6564 && id < 604 || id > 13295 && id < 13299) {
				...
				case 7780:
				case 1201:
					player.getSlayer().handleDialouge(2007);
					break;
				case 4287:
		...
		World.getWorld().submitAreaEvent(player, new CoordinateEvent(player, locationToWalk.getX(), locationToWalk.getX(), npc.getDefinition().getCacheDefinition().size, npc.getDefinition().getCacheDefinition().size) {
			...
			public void execute() {
				...
				switch (id) {
					...
					case 7780:
					case 1201:
						player.getSlayer().handleDialouge(2004);
					break;
				}
			}
		});
	...
	private void option1(final Player player, Message in) {
		...
		World.getWorld().submitAreaEvent(player, new CoordinateEvent(player, locationToWalk.getX(), locationToWalk.getX(), npc.getDefinition().getCacheDefinition().size, npc.getDefinition().getCacheDefinition().size) {
			...
			public void execute() {
				...
				switch (id) {
				case 650:
					World.getWorld().getShopManager().openShop(player, 650);
					break;
				case 290:
					World.getWorld().getShopManager().openShop(player, 290);
					break;
				case 534:
					World.getWorld().getShopManager().openShop(player, 534);
					break;
				case 9711:
					DialogueManager.proceedDialogue(player, 9125);
					break;
				case 9462:
					player.turnTo(npc);
					player.animate(4278);
