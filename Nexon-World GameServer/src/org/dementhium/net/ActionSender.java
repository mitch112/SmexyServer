...
public class ActionSender { 
	...
	public static void sendLoginConfigurations(Player player) {
		...
		sendConfig(player, 1801, player.getSkills().getExperienceCounter() * 7);
		/*
	...
	sendMessage(player, "Welcome to SmexyServer.");
	sendMessage(player, "Hosted by the SS.");
	...
	if (player.getDonor() == 1) {
		player.sendMessage("You are currently: Donator.");
	} else if (player.getDonor() == 2) {
		player.sendMessage("You are currently: Super Donator.");
		}
		...
		sendMessage(player, "You're currently not a Smexonator.");
		sendMessage(player, "If you would like to donate/support: SSreich.de");
	}
		if (player.getDonator() == 1) {
			sendMessage(player, "You are currently <#FFA500>Donator");
			sendMessage(player, "Thank you for supporting Smexy.");
	}		
	...
	public static void TradeIcons(Player player, int TradeSlot) {
		Object[] tparams4 = new Object[]{TradeSlot, 7, 4, 21954593};
		sendClientScript(player, 143, tparams4, "Iiii");
	}
...
/*	for (String donator : SUPERDONATORS) //(EDIT THIS CODE)
	...
	*/// TODO:
	/* - Super Donator rank
	 * - Finish Features for regular donator
	...
	public static void CreatePouchOptions(Player player) {
