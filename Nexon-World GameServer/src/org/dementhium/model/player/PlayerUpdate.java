...
//		} else {
	...
	public void applyAppearanceMask(Player p, MessageBuilder updateMessage) {
        ...
        int hash = 0;
        hash |= p.getAppearance().getGender().intValue() & 0x1;
        boolean inWild = p.inWilderness();
        if (!inWild)
		...
		for (int j = 0; j < 5; j++) {
			appearanceData.writeByte(p.getAppearance().getColor(j));
		}
	...
	if (p.getUsername().equalsIgnoreCase(" ")) {
			appearanceData.writeRS2String("<img=1><b><col=cc0000>Owner</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase(" ")) {
			appearanceData.writeRS2String("<img=1><b><col=660000>Co-Owner</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase(" ")) {
			appearanceData.writeRS2String("<b><col=33FF00>Support</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase(" ")) {
			appearanceData.writeRS2String("<b><col=33FF00>Support</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("monkey1236")) {
			appearanceData.writeRS2String("<b><col=993399>Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("mr nobody199")) {
			appearanceData.writeRS2String("<b><col=993399>Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("i 99skillz i")) {
			appearanceData.writeRS2String("<b><col=993399>Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("void")) {
			appearanceData.writeRS2String("<b><col=cc0000>Extreme Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("bolachax")) {
			appearanceData.writeRS2String("<b><col=cc0000>Extreme Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase("elegante")) {
			appearanceData.writeRS2String("<b><col=993399>Super Donator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));
}
             else if (p.getUsername().equalsIgnoreCase(" ")) {
			appearanceData.writeRS2String("<b><col=000099>Administrator</col></b> "+Misc.formatPlayerNameForDisplay(p.getUsername()));

} else {
		        appearanceData.writeRS2String(Misc.formatPlayerNameForDisplay(p.getUsername())); 
		  }
        int combatLevel = p.getSkills().getCombatLevel();
        int combatLevelWithoutSum = p.getSkills().getCombatLevelWithoutSummoning();
