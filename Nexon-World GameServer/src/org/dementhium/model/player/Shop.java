...
public class Shop {
	...
	public static final int[] unsellableItems = new int[]{995, 11180, 19864, 6529, 5020};
	public static int[] donatorOnly = {20135, 20136, 20137, 20138, 20139, 20140, 20141, 20142, 20143, 20144, 20145, 20146};
	...
	public Shop(int id, int currency, boolean isGeneralStore, int[] items, int[] amounts, boolean inGame) {
		this.shopId = id;
 ...
 * ActionSender.sendConfig(player, 118, 3);
		...
		ActionSender.sendConfig(player, 532, currency);
 ...
 */
		...
		case 621:
			...
			case 0:
				...
				case 6:
					...
					p.sendMessage(def.getName() + " costs " + formatPrice(def.getStorePrice()) + " " + ItemDefinition.forId(currency).getName() + ".");
	...
	private void sellItem(Player p, int itemId, int amount) {
		...
		if (def.getStorePrice() > 0) {
			...
			if (!p.getInventory().hasRoomFor(currency, price)) {
	...
	private void buyItem(Player p, int id, int amount) {
		...
		if (p.getInventory().getContainer().getItemCount(currency) < price) {
