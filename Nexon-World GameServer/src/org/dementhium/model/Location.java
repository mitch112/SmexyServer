...
public int getWildernessLevel() {
    ...
    public boolean inFightCaves(Player p) {
        int absX = p.getLocation().getX();
        int absY = p.getLocation().getY();
        return (absX >= 2367 && absX <= 2433 && absY >= 5056 && absY <= 5119);
      }
	public boolean inSafePk(Player p) {
        int absX = p.getLocation().getX();
        int absY = p.getLocation().getY();
        return (absX >= 2948 && absX <= 2875 && absY >= 5512 && absY <= 5523);
	}
