...
public class EarthSurge extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		if (interaction.getSource().getPlayer().getSkills().getLevel(Skills.MAGIC) < 90) {
			interaction.getSource().getPlayer().sendMessage("You need a level of 90 Magic to use earth surge.");
			return false;
		}
