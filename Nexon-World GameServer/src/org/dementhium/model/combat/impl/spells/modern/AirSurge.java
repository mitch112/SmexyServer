...
public class AirSurge extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		if (interaction.getSource().getPlayer().getSkills().getLevel(Skills.MAGIC) < 81) {
			interaction.getSource().getPlayer().sendMessage("You need a level of 81 Magic to use air surge.");
			return false;
		}
