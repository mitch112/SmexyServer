...
public class ShadowBarrage extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		...
		if (interaction.getSource().getPlayer().getSkills().getLevel(Skills.MAGIC) < 88) {
			interaction.getSource().getPlayer().sendMessage("You need a level of 88 Magic to use shadow barrage.");
			return false;
		}
