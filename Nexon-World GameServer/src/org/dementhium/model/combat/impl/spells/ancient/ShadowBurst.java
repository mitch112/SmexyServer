...
public class ShadowBurst extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		if (interaction.getSource().getPlayer().getSkills().getLevel(Skills.MAGIC) < 64) {
			interaction.getSource().getPlayer().sendMessage("You need a level of 64 Magic to use shadow burst.");
			return false;
		}
