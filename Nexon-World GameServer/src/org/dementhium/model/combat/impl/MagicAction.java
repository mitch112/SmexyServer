...
public class MagicAction extends CombatAction {
	...
	public static boolean checkRunes(Mob mob, Item[] runes, boolean remove) {
		...
		if (remove) {
			...
			} else if (weapon != null && weapon.getDefinition().getName().startsWith("Celestial catalytic staff")) {
				if (Misc.random(1, 8) < 3) {
					mob.getPlayer().sendMessage("Your staff is drawing dark magic.");
					mob.getPlayer().graphics(1935);
					mob.getPlayer().getSkills().Modify(6, 0.2, 1, false);
				}
			} else if (weapon != null && weapon.getDefinition().getName().startsWith("Armadyl crozier")) {
				if (Misc.random(1, 10) < 3) {
					mob.getPlayer().sendMessage("You feel restored.");
					mob.getPlayer().graphics(1930);
					int modification = (int) Math.floor(1 + (mob.getPlayer().getSkills().getLevelForExperience(6) * 0.05));
					mob.getPlayer().getSkills().increaseLevelToMaximumModification(6, modification);
				}
			}
	...
	private static boolean hasStaff(Player player, int rune) {
		...
		Item book = player.getEquipment().get(player.getEquipment().SLOT_SHIELD);
		if (weapon == null) {
			return false;
		}
		if (book == null) {
		...
		for (int id : staves) {
			if (weapon.getId() == id || book.getId() == id) {
				return true;
			}
		}
		return false;
