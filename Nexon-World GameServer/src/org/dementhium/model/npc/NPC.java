...
public class NPC extends Mob {
    ...
    public static final int[] UNRESPAWNABLE_NPCS = {
            2746,  1532, 1533, 1534, 1535,
             8127, 3851, 907, 908, 911, 11301, 11302,
            5384, 4278, 4279, 4280, 4281, 4282, 4283, 4284
    };
	
	public int[] SPECIAL = {10039, 907, 908, 911, 11301, 11302, 2745, 8133};
   ...
   public void walkTo(int moveX, int moveY) {
	...
	private int addCharms() {
		int id = Misc.random(1, 4);
		switch (id) {
			case 1: return 12158;
			case 2: return 12159;
			case 3: return 12160;
	...
	public void dropCharms(Player player) {
			GroundItemManager.createGroundItem(new GroundItem(player, new Item(12183, Misc.random(20, 50)), getLocation().getX(), getLocation().getY(),
						getLocation().getZ(), false));
			GroundItemManager.createGroundItem(new GroundItem(player, new Item(addCharms(), 1), getLocation().getX(), getLocation().getY(),
						getLocation().getZ(), false));
	}
	
	public void addDrop(Player player, int itemId, int amount, boolean bool) {
		GroundItemManager.createGroundItem(new GroundItem(player, new Item(itemId, amount), getLocation().getX(), getLocation().getY(),
						getLocation().getZ(), bool));
	}
    ...
    public void sendDead() {
		...
		if (getDefinition().getId() == 13447) {
			mask.setSwitchId(13448);
			forceText("Taste my wrath!");
			for (Player p : Region.getLocalPlayers(getLocation(), 5)) {
				Prayer.wrathEffect(this, p);
			}
		}
		...
		switch (npcId) {
			...
			case 10204:
			case 10203:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 11);
				break;
			case 12940:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 9);
				break;
			case 12841:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 20);
				break;
			case 11886:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 15);
				break;
			case 11751:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 12);
				break;
			case 10860:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 5);
				break;
			case 10409:
				killer.getPlayer().getSkills().addExperience(24, killer.getPlayer().getSkills().getLevel(24) * 3);
				break;
		}
		World.getWorld().submit(new Tick(deathTick) {
			...
			public void execute() {
				...
				if (killer != null) {
					...
					if (getId() == 907) {
						killer.getPlayer().addKolodion(killer.getPlayer(), 908);
					} else if (getId() == 908) {
						killer.getPlayer().addKolodion(killer.getPlayer(), 911);
					} else if (getId() == 911) {
						killer.getPlayer().addKolodion(killer.getPlayer(), 11301);
					} else if (getId() == 11301) {
						killer.getPlayer().addKolodion(killer.getPlayer(), 11302);
					} else if (getId() == 11302) {
						killer.getPlayer().teleport(2965, 3378, 0);
						DialogueManager.proceedDialogue(killer.getPlayer(), 79);
					}
					if (isSpecial()) {
						//Do nothing
					} else {
						dropCharms(killer.getPlayer());
					}
		...
		World.getWorld().submit(new Tick(deathTick) {
            @Override
            public void execute() {
             if (id == 2745 && (killer.getPlayer().getLocation().inFightCaves(killer.getPlayer()))) {
             FightCaves.teleFromCave(killer.getPlayer());
              resetCombat();
                 setHidden(false);
                 setHp(getMaxHp());
                 setDead(true);
	...
	public boolean isAttackable(Mob mob) {
		...
		if (lastAttacker != null && lastAttacker != mob && (!isMulti() || !mob.isMulti())) {
			...
			if(player.getSkills().getLevel(Skills.SLAYER) < 85 && getId() == 1615){
				...
				return false;
			} else if(player.getSkills().getLevel(Skills.SLAYER) < 77 && getId() == 9465) {
				player.sendMessage("You need a slayer level of 77 to attack this creature.");
				return false;
			} else if(player.getSkills().getLevel(Skills.SLAYER) < 73 && getId() == 9466) {
				player.sendMessage("You need a slayer level of 73 to attack this creature.");
				return false;
			} else if(player.getSkills().getLevel(Skills.SLAYER) < 73 && getId() == 9467) {
				player.sendMessage("You need a slayer level of 73 to attack this creature.");
				return false;
			} else if(player.getSkills().getLevel(24) < 75 && getId() == 11751) {
				player.sendMessage("You need a dungeoneering level of 75 to attack this creature.");
				return false;
			} else if(player.getSkills().getLevel(24) < 113 && getId() == 12841) {
				player.sendMessage("You need a dungeoneering level of 113 to attack this creature.");
				return false;
			} else if(player.getSkills().getLevel(24) < 45 && getId() == 12940) {
				player.sendMessage("You need a dungeoneering level of 45 to attack this creature.");
				return false;
			} else if (player.getSkills().getLevel(24) < 75 && getId() == 10206 ||
						player.getSkills().getLevel(24) < 75 && getId() == 10205 ||
						player.getSkills().getLevel(24) < 75 && getId() == 10204 ||
						player.getSkills().getLevel(24) < 75 && getId() == 10203) {
				player.sendMessage("You need a dungeoneering level of 75 to attack this creature.");
				return false;
			}
		}
		...
		if (source.isPlayer() && source.getPlayer().getSlayer() != null && source.getPlayer().getSlayer().getSlayerTask() != null) {
			if (source.getPlayer().getSlayer().getSlayerTask().getName().equalsIgnoreCase(definition.getCacheDefinition().getName())) {
				...
				if (item != null && item.getDefinition().getName().contains("lack mask")) {
					hit *= 1.167;
				} else if (item != null && item.getDefinition().getName().contains("layer mask")) {
					hit *= 1.20;
				}
			}
		}
