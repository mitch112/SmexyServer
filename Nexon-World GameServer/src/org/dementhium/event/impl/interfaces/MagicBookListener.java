...
public class MagicBookListener extends EventListener {
    ...
    public boolean interfaceOption(final Player player, int interfaceId, int buttonId, int slot, int itemId, int opcode) {
		...
		int weapon = player.getEquipment().getSlot(3);
		if (modern && buttonId == 67 && !player.hasArmadyl()) {
			ActionSender.sendMessage(player, "Talk to Kolodion to learn how to use this spell.");
			return true;
		}
		if (modern && buttonId == 67 && weapon != 19362) {
			ActionSender.sendMessage(player, "You you need to be wielding the Armadyl crozier to cast this spell.");
			return true;
		}
